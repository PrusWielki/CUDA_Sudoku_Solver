...
__global__ void backtrack(int * boards_set, int amount_of_boards, int * not_filled_cells, int * amount_nfc, int * solved_board, int * done) {
  ...
  while ((index < amount_of_boards) && (0 == * done)) { //loop continues until we run out of boards to fill in or we find a solution
    ...
    while ((index_of_empty >= 0) && (index_of_empty < current_amount_nfc)) { //loops until the index goes beyond the board
...
__global__ void kernel_bfs(int * boards_old, int * boards_new, int amount_of_boards, int * board_index, int * not_filled_cells, int * amount_nfc) {
  ...
  while (index < amount_of_boards) { //repeat the process until we run out of boards
    int empty_spot_found = 0;

    for (int i = (index * BOARDSIZE * BOARDSIZE);
      (i < (index * BOARDSIZE * BOARDSIZE) + BOARDSIZE * BOARDSIZE) && (empty_spot_found == 0); i++) { //go through all the cells of current board
      ...
      if (boards_old[i] == 0) {
        ...
        for (int attempt = 1; attempt <= BOARDSIZE; attempt++) { //tries to find all possible values in current cell (1-9)
          int works = 1;
          ...
          if (works == 1) {
            ...
            for (int r = 0; r < BOARDSIZE; r++) {
              for (int c = 0; c < BOARDSIZE; c++) {
                ...
                if (boards_old[index * BOARDSIZE * BOARDSIZE + r * BOARDSIZE + c] == 0 && (r != row || c != collumn)) { //fiunds empty cells, excluding the one we are filling right now
...
int main(int argc, char ** argv) {
  ...
  backtrack << < maxBlocks, threadsPerBlock >>>
    (boards_new, host_count, not_filled_cells, amount_nfc, dev_solved, dev_finished);
